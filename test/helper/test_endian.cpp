...
TEST_F(EndianTest, HostToNetwork16) {
  ...
  uint16_t network_value = endian::HostToNetwork16(host_bytes.data());
...
TEST_F(EndianTest, HostToNetwork32) {
  ...
  std::vector<uint8_t> host_bytes = {0x42, 0xA4, 0x12,
                                     0x21};  // リトルエンディアン形式
  uint32_t network_value = endian::HostToNetwork32(host_bytes.data());

  // ネットワークバイトオーダー（ビッグエンディアン）では
  // バイト順が逆転される（リトルエンディアンシステムの場合）
  EXPECT_EQ((network_value >> 24U) & 0xFFU, 0x42);  // 最上位バイト
  EXPECT_EQ((network_value >> 16U) & 0xFFU, 0xA4);  // 2番目バイト
  EXPECT_EQ((network_value >> 8U) & 0xFFU, 0x12);   // 3番目バイト
  EXPECT_EQ(network_value & 0xFFU, 0x21);           // 最下位バイト
}
...
TEST_F(EndianTest, NetworkToHost16) {
  ...
  uint16_t host_value = endian::NetworkToHost16(bytes.data());
...
TEST_F(EndianTest, NetworkToHost32) {
  ...
  uint32_t host_value = endian::NetworkToHost32(bytes.data());
...
TEST_F(EndianTest, ConversionSymmetry16bit) {
  ...
  std::vector<uint8_t> host_bytes = {
      ...
      static_cast<uint8_t>(original >> 8U)};  // リトルエンディアン形式

  // host→network→hostで元に戻ることを確認
  uint16_t network = endian::HostToNetwork16(host_bytes.data());

  // networkの値をバイト配列として解釈（ネットワークバイトオーダー）
  uint8_t* bytes = reinterpret_cast<uint8_t*>(&network);

  uint16_t restored = endian::NetworkToHost16(bytes);
...
TEST_F(EndianTest, ConversionSymmetry32bit) {
  ...
  // 元の値をホストバイトオーダーのバイト配列として準備
  std::vector<uint8_t> host_bytes = {
      static_cast<uint8_t>(original), static_cast<uint8_t>(original >> 8U),
      static_cast<uint8_t>(original >> 16U),
      static_cast<uint8_t>(original >> 24U)};  // リトルエンディアン形式

  // host→network→hostで元に戻ることを確認
  uint32_t network = endian::HostToNetwork32(host_bytes.data());

  // networkの値をバイト配列として解釈（ネットワークバイトオーダー）
  uint8_t* bytes = reinterpret_cast<uint8_t*>(&network);

  uint32_t restored = endian::NetworkToHost32(bytes);
