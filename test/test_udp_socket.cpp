...
protected:
    ...
    void TearDown() override
    {
        ...
        if (test_duration.count() > 5000) // 5秒以上
        {
            std::cerr << "Warning: Test took " << test_duration.count()
                      << "ms (>5s), potential deadlock or performance issue" << std::endl;
        }
    ...
    bool waitForPackets(Container &received_data, std::mutex &data_mutex,
                        size_t expected_count, std::chrono::milliseconds timeout = std::chrono::milliseconds(3000))
    {
        ...
        while (std::chrono::steady_clock::now() - start_wait < timeout)
        {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(10)); // 短い間隔でチェック
        }
...
TEST_F(UDPSocketTest, CanReceiveUDPPacket)
{
    ...
    bool receive_result = receiver.receiveFrom(received_data, sender_ip, sender_port, 1000); // 1秒タイムアウト
...
TEST_F(UDPSocketTest, MultiPacketDataIntegrityTest)
{
    ...
    std::vector<std::vector<uint8_t>> test_packets = {
        ...
        {'L', 'a', 'r', 'g', 'e', 'r', ' ', 'p', 'a', 'c', 'k', 'e', 't'}, // 大きなパケット
    ...
    {
        bool send_result = sender.sendTo(test_packets[i], "127.0.0.1", receiver_port);
    ...
    for (size_t i = 0; i < max_expected; ++i)
    {
        ...
        bool receive_result = receiver.receiveFrom(received_data, sender_ip, sender_port, 1000);
    ...
    for (const auto &received_packet : received_packets)
    {
        ...
        EXPECT_TRUE(found_match) << "Received packet that doesn't match any sent packet";
    }
    ...
    if (received_packets.size() == test_packets.size())
    {
        ...
        for (const auto &sent_packet : test_packets)
        {
            ...
            EXPECT_TRUE(found_in_received) << "A sent packet was not found in received packets";
        }
    }
}
...
TEST_F(UDPSocketTest, ConcurrentSendTest)
{
    ...
    {
        threads.emplace_back([this, &sender, &send_results, &sent_data, &sent_data_mutex,
                              t, sends_per_thread, receiver_port]()
                             {
            for (int i = 0; i < sends_per_thread; ++i)
            {
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(10)); // 少し間隔を空ける
            } });
    }
    ...
    std::thread receiver_thread([&receiver, &received_data, &received_data_mutex, &stop_receiving]()
                                {
        while (!stop_receiving.load())
        {
            ...
            bool received = receiver.receiveFrom(packet_data, sender_ip, sender_port, 100); // 100ms timeout
    ...
    bool received_all = waitForPackets(received_data, received_data_mutex, expected_packets);
    ...
    if (!received_all)
    {
        std::cout << "Warning: Did not receive all expected packets within timeout. "
                  << "Expected: " << expected_packets << ", Received: " << received_data.size() << std::endl;
    }
    ...
    EXPECT_EQ(sent_data.size(), num_threads * sends_per_thread) << "Not all data was recorded as sent";
...
TEST_F(UDPSocketTest, DeadlockDetectionMixedOperations)
{
    ...
    std::thread timeout_thread([&]()
        ...
        while (test_should_continue.load())
        {
            ...
            // 3秒でタイムアウト
            if (elapsed.count() >= 3 && completed_operations.load() < num_threads * operations_per_thread)
            {
    ...
    for (int t = 0; t < num_threads; ++t)
    {
        threads.emplace_back([&socket, &completed_operations, &sent_packets, &received_packets,
                              &data_mutex, t, operations_per_thread, socket_port]()
                             {
            for (int i = 0; i < operations_per_thread; ++i)
            {
                ...
                case 2: // 受信（内容を記録）
                {
                    ...
                    std::string sender_ip;
                    int sender_port;
                    bool received = socket.receiveFrom(received_data, sender_ip, sender_port, 5); // 5ms短時間
                ...
                case 3: // 別のポートに送信
                {
                    std::vector<uint8_t> data = {'E', 'x', 't', static_cast<uint8_t>('0' + t)};
                    socket.sendTo(data, "127.0.0.1", 25000 + t);
                    break;
                }
                }
                completed_operations.fetch_add(1);
                std::this_thread::sleep_for(std::chrono::microseconds(200)); // 少し時間を増やす
            } });
    }
    ...
    // デッドロックが発生していないことを確認
    EXPECT_FALSE(deadlock_detected.load()) << "Deadlock detected in mixed operations";
    EXPECT_EQ(completed_operations.load(), num_threads * operations_per_thread) << "Not all operations completed";
    ...
    for (const auto &received_packet : received_packets)
    {
        ...
        EXPECT_TRUE(found_match) << "Received packet that doesn't match any sent packet in mixed operations";
    }
}
...
TEST_F(UDPSocketTest, DeadlockDetectionNestedCalls)
{
    ...
    EXPECT_EQ(completed_calls.load(), expected_calls) << "Not all nested calls completed";
}
...
TEST_F(UDPSocketTest, DeadlockStressTest)
{
    ...
    std::thread timeout_thread([&]()
        ...
        while (test_should_continue.load())
        {
            ...
            int expected = num_sockets * num_threads_per_socket * operations_per_thread;
            if (elapsed.count() >= 3 && total_operations.load() < expected * 0.9) // 3秒でタイムアウト
            {
                ...
                for (int op = 0; op < operations_per_thread; ++op)
                {
                    try
                    {
                        ...
                        case 0: // Cross-socket communication
                        {
                            ...
                            std::vector<uint8_t> data = {'S', static_cast<uint8_t>('0' + s), '>', static_cast<uint8_t>('0' + target_socket)};
                            socket->sendTo(data, "127.0.0.1", sockets[target_socket]->getLocalPort());
                            break;
                        }
                        case 1: // Status check
                        {
                        ...
                        case 2: // Self-send
                        {
                            std::vector<uint8_t> data = {'S', 'e', 'l', 'f', static_cast<uint8_t>('0' + s)};
                        ...
                        case 3: // Quick receive attempt
                        {
                            ...
                            std::string sender_ip;
                            int sender_port;
                            socket->receiveFrom(received_data, sender_ip, sender_port, 1); // 1ms
                            break;
                        }
                        }
    ...
    // ストレステスト結果を確認
    int expected_operations = num_sockets * num_threads_per_socket * operations_per_thread;
    EXPECT_FALSE(stress_test_failed.load()) << "Stress test failed due to timeout or exceptions";
    EXPECT_GE(total_operations.load(), expected_operations * 0.9)
        << "Less than 90% of operations completed. Completed: " << total_operations.load()
...
TEST_F(UDPSocketTest, TimedConcurrentTest)
{
    ...
    for (int t = 0; t < num_threads; ++t)
    {
        threads.emplace_back([&sender, &results, &sent_data, &sent_data_mutex,
                              t, operations_per_thread, sleep_ms_per_operation, receiver_port]()
                             {
            for (int i = 0; i < operations_per_thread; ++i)
            {
                std::vector<uint8_t> data = {
                ...
                // 明示的に時間を消費
                std::this_thread::sleep_for(std::chrono::milliseconds(sleep_ms_per_operation));
            } });
    }
    ...
    std::thread receiver_thread([&receiver, &received_data, &received_data_mutex, &stop_receiving]()
        ...
        while (!stop_receiving.load())
        {
            ...
            bool received = receiver.receiveFrom(packet_data, sender_ip, sender_port, 100);
            if (received && !packet_data.empty())
            {
                std::lock_guard<std::mutex> lock(received_data_mutex);
                received_data.push_back(packet_data);
    ...
    // 期待される最小時間：operations_per_thread * sleep_ms_per_operation（並列実行なので）
    int expected_min_time = operations_per_thread * sleep_ms_per_operation;

    EXPECT_GE(elapsed.count(), expected_min_time * 0.8)
        << "Test completed too quickly. Expected at least " << expected_min_time * 0.8
        << "ms but took " << elapsed.count() << "ms";
    ...
    for (const auto &received_packet : received_data)
    {
        ...
        EXPECT_TRUE(found_match) << "Received packet that doesn't match any sent packet in timed test";
    }
}
...
TEST_F(UDPSocketTest, TimeoutDetectionTest)
{
    ...
    std::thread timeout_thread([&]()
        ...
        while (test_should_continue.load())
        {
            ...
            // 500msでタイムアウト
            if (elapsed.count() >= 500 && completed_operations.load() < target_operations)
            {
                deadlock_detected.store(true);
                break;
            }
    ...
    std::thread worker_thread([&]()
        ...
        // 残りの操作はタイムアウト後に実行
        while (completed_operations.load() < target_operations && !deadlock_detected.load())
        {
            completed_operations.fetch_add(1);
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        } });
    ...
    EXPECT_TRUE(deadlock_detected.load()) << "Timeout detection did not work properly";
    EXPECT_LT(completed_operations.load(), target_operations) << "All operations completed unexpectedly";
}
